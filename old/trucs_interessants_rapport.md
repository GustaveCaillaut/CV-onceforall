...
## 6. Différence avec l’élasticité de largeur du papier OFA

Dans le papier OFA original, l’élasticité de largeur peut aussi concerner le nombre de channels de sortie des blocs.

Dans notre implémentation, les channels de sortie des stages sont fixés :

```python
self.stage_out_channels = [32, 48, 64, 96, 128]
```

Ce qui varie est uniquement la largeur interne des MBConv via l’expansion ratio.

Cette simplification a été choisie pour plusieurs raisons :

1. **Stabilité du code**  
   Si les channels de sortie variaient, tous les blocs suivants devraient accepter des tailles d’entrée variables plus complexes.

2. **BatchNorm plus simple**  
   Le slicing des statistiques BatchNorm reste raisonnable à gérer.

3. **Résidus plus simples**  
   Les connexions résiduelles nécessitent des dimensions compatibles. Garder les channels de sortie fixes évite beaucoup de cas particuliers.

4. **Projet pédagogique**  
   L’objectif est d’étudier le principe OFA et les stratégies de sampling, pas de reproduire exactement toute l’ingénierie MobileNetV3/OFA.

Ce choix réduit la richesse de l’espace de recherche, mais conserve les mécanismes essentiels :

- weight sharing ;
- progressive shrinking ;
- kernel elasticity ;
- depth elasticity ;
- width interne via expansion ratio ;
- accuracy predictor ;
...
| Width elasticity | Plus complète | Expansion ratio seulement |
...
### 18.3 L’expansion ratio est une forme simplifiée de width

Notre code ne modifie pas les output channels, mais modifie la largeur interne des MBConv.  
C’est moins général que le papier, mais suffisant pour étudier les effets du sampling.
