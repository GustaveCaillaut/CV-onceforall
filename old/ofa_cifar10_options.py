...
def set_seed(seed: int) -> None:
    ...
    # Default: deterministic-ish behavior. Some evaluation presets below can
    # turn cudnn.benchmark back on for speed.
...
# -----------------------------------------------------------------------------
# Evaluation / sampling option system
# -----------------------------------------------------------------------------

# These presets control how subnet accuracies are measured. They are intentionally
# dictionary-based so you can quickly add/remove options while experimenting.
#
# Important options:
# - cache_*_on_gpu: preloads the first N batches on GPU once, then reuses them
#   for every architecture. This removes DataLoader overhead from the inner loop.
# - pre_resize_cache: stores 24/28/32 versions of each cached image batch. The
#   model otherwise interpolates inside every forward.
# - bn_calib_loader: "train" uses augmented train batches; "val" uses clean
#   non-augmented train-val batches. For BN calibration, "val" is often faster
#   and more stable for debugging.
# - restore_bn_after_eval: saves/restores BN statistics around each subnet eval.
#   Usually not needed in sample_acc_dataset because the next subnet resets BN
#   anyway, but useful for diagnostics.
EVAL_OPTION_PRESETS: Dict[str, Dict[str, Any]] = {
    "no_opti": {
...
def parse_eval_options(preset: str = "balanced", options_json: Optional[str] = None) -> Dict[str, Any]:
    """Return an option dictionary, optionally overridden by JSON.

    `options_json` can be either a JSON string, e.g.
        '{"cache_eval_on_gpu": false}'
    or a path to a JSON file containing a dictionary.
    """
    if preset not in EVAL_OPTION_PRESETS:
        raise ValueError(f"Unknown eval preset {preset!r}. Choices: {list(EVAL_OPTION_PRESETS)}")
    opts = dict(EVAL_OPTION_PRESETS[preset])
    if options_json:
        maybe_path = Path(options_json)
        if maybe_path.exists():
            overrides = json.loads(maybe_path.read_text())
        else:
            overrides = json.loads(options_json)
        if not isinstance(overrides, dict):
            raise ValueError("--eval-options must decode to a JSON dictionary")
        opts.update(overrides)
    torch.backends.cudnn.benchmark = bool(opts.get("cudnn_benchmark", False))
    return opts


def _select_images_for_spec(images: Any, spec: Optional[Dict]) -> torch.Tensor:
    """Select the right tensor from a cached pre-resized batch, if needed."""
    if isinstance(images, dict):
        if spec is None:
            # Fallback to the largest cached resolution.
            key = max(int(k) for k in images.keys())
        else:
            key = int(spec["r"])
        return images[key]
    return images
...
def cache_batches_on_gpu(
    ...
    """Preload the first `max_batches` batches on GPU.

    If pre_resize=True, each image batch is stored as a dict {resolution: tensor}.
    This avoids repeated F.interpolate calls in OFACifarNet.forward.
    """
    cached = []
    for i, batch in enumerate(loader):
        ...
        if pre_resize:
            ...
            for r in resolutions:
                r = int(r)
                if images.size(-1) == r and images.size(-2) == r:
                    image_obj[r] = images
                else:
                    image_obj[r] = F.interpolate(images, size=(r, r), mode="bilinear", align_corners=False)
        else:
            image_obj = images
        if include_targets:
            cached.append((image_obj, targets.to(device, non_blocking=True)))
        else:
            cached.append((image_obj, None))
    return cached


def clone_bn_state(model: nn.Module) -> Dict[str, torch.Tensor]:
    """Clone only BN running stats for optional restoration after a subnet eval."""
    out = {}
    for name, m in model.named_modules():
        if isinstance(m, DynamicBatchNorm2d):
            out[name + ".running_mean"] = m.bn.running_mean.detach().clone()
            out[name + ".running_var"] = m.bn.running_var.detach().clone()
            out[name + ".num_batches_tracked"] = m.bn.num_batches_tracked.detach().clone()
        elif isinstance(m, nn.BatchNorm2d):
            out[name + ".running_mean"] = m.running_mean.detach().clone()
            out[name + ".running_var"] = m.running_var.detach().clone()
            out[name + ".num_batches_tracked"] = m.num_batches_tracked.detach().clone()
    return out


def restore_bn_state(model: nn.Module, state: Dict[str, torch.Tensor]) -> None:
    for name, m in model.named_modules():
        if isinstance(m, DynamicBatchNorm2d):
            prefix = name + "."
            m.bn.running_mean.copy_(state[prefix + "running_mean"])
            m.bn.running_var.copy_(state[prefix + "running_var"])
            m.bn.num_batches_tracked.copy_(state[prefix + "num_batches_tracked"])
        elif isinstance(m, nn.BatchNorm2d):
            prefix = name + "."
            m.running_mean.copy_(state[prefix + "running_mean"])
            m.running_var.copy_(state[prefix + "running_var"])
            m.num_batches_tracked.copy_(state[prefix + "num_batches_tracked"])
...
class DynamicDepthwiseConv2d(nn.Module):
    """Depthwise convolution with dynamic channels and OFA-style kernel transforms.

    We store a maximum 7x7 depthwise kernel. Smaller kernels are obtained by:
      7x7 -> center crop 5x5 -> learned 25x25 transform
      5x5 -> center crop 3x3 -> learned 9x9 transform

    The transform matrices are shared across channels, as in OFA.
    """

    def __init__(self, max_channels: int, max_kernel_size: int = 7):
        super().__init__()
        if max_kernel_size != 7:
            raise ValueError("This implementation assumes max_kernel_size=7.")
        ...
        # OFA-style kernel transformation matrices.
        # They are shared across all channels of this layer.
        #
        # F.linear(x, W) computes x @ W.T.
        # Since W is initialized to identity, the initial behavior is exactly
        # equivalent to center-crop. Training can then learn a better transform.
    ...
    def get_active_filter(self, kernel_size: int) -> torch.Tensor:
        """Return the active depthwise kernel tensor.
        ...
        # Apply learned 25x25 transform, shared across channels.
        c = weight_5.size(0)
        ...
        # Apply learned 9x9 transform, shared across channels.
...
def make_frozen_teacher(model: OFACifarNet, device: torch.device) -> OFACifarNet:
    """Return a frozen copy of the current largest-subnet model.

    Avoid copy.deepcopy(model): on some Windows/PyTorch/CUDA setups it can
    be extremely slow or look like an infinite hang. Instead, instantiate a
    fresh model and copy the state_dict explicitly through CPU tensors.
    """
    teacher = OFACifarNet(
        num_classes=model.num_classes,
        search_space=copy.deepcopy(model.search_space),
    )
    cpu_state = {k: v.detach().cpu().clone() for k, v in model.state_dict().items()}
    teacher.load_state_dict(cpu_state, strict=True)
...
def evaluate(
    ...
    """Evaluate one active subnet on a loader or cached GPU batch list."""
    model.eval()
    if spec is not None:
    ...
    for i, (images, targets) in enumerate(loader):
        if max_batches is not None and i >= max_batches:
            break
        images = _select_images_for_spec(images, spec).to(device, non_blocking=True)
        targets = targets.to(device, non_blocking=True)
        logits = model(images)
...
def calibrate_bn(
    ...
    Uses a cumulative moving average implemented with momentum=1/(i+1). This is
    compatible with DynamicBatchNorm2d because its forward uses F.batch_norm,
    which requires momentum to be a float rather than None.
    """
    if num_batches <= 0:
        return
    ...
    for i, (images, _) in enumerate(loader):
        if i >= num_batches:
            break
        current_momentum = 1.0 / float(i + 1)
        for m in model.modules():
            if isinstance(m, DynamicBatchNorm2d):
                ...
                continue
            if bn not in old_momenta:
                old_momenta[bn] = bn.momentum
            bn.momentum = current_momentum

        images = _select_images_for_spec(images, spec).to(device, non_blocking=True)
        _ = model(images)

    for bn, old_momentum in old_momenta.items():
...
def train_accuracy_predictor(
    ...
    plt.plot(val_rmse_history)
    plt.xlabel("Epoch")
    plt.ylabel("Validation RMSE")
    plt.title("Accuracy Predictor Validation RMSE")
    plt.grid()
    plt.savefig(out_dir / "accuracy_predictor_val_rmse.png")
...
def sample_accuracy_dataset(
    ...
    eval_options: Optional[Dict[str, Any]] = None,
) -> None:
    ...
    eval_options = eval_options or parse_eval_options("balanced")
    print(f"Eval options: {json.dumps(eval_options, sort_keys=True)}")

    # Optional GPU caching. These cached lists can be passed to calibrate_bn/evaluate
    # exactly like normal loaders, but they avoid DataLoader overhead in the inner loop.
    calib_loader: Iterable = train_loader if eval_options.get("bn_calib_loader", "train") == "train" else val_loader
    eval_loader: Iterable = val_loader
    if bn_calib_batches > 0 and eval_options.get("cache_calib_on_gpu", False):
        calib_loader = cache_batches_on_gpu(
            calib_loader,
            device=device,
            max_batches=bn_calib_batches,
            resolutions=ss.resolutions,
            pre_resize=bool(eval_options.get("pre_resize_cache", False)),
            include_targets=False,
        )
        print(f"Cached {len(calib_loader)} BN-calibration batches on GPU.")
    if eval_options.get("cache_eval_on_gpu", False):
        eval_loader = cache_batches_on_gpu(
            eval_loader,
            device=device,
        ...
        print(f"Cached {len(eval_loader)} validation batches on GPU.")
        eval_max_batches = None  # already truncated by cache_batches_on_gpu
    else:
        eval_max_batches = val_batches
    ...
    with open(output_path, "w") as f:
        for i in range(num_arch_samples):
            ...
            # accuracy estimates. Optionally save/restore BN stats for diagnostics.
            t0 = time.perf_counter()
            bn_state = clone_bn_state(model) if eval_options.get("restore_bn_after_eval", False) else None
            if bn_calib_batches > 0:
                calibrate_bn(model, calib_loader, device, spec, num_batches=bn_calib_batches)
            t_calib = time.perf_counter()
            metrics = evaluate(model, eval_loader, device, spec=spec, max_batches=eval_max_batches)
            ...
            f.flush()
            if eval_options.get("timing", False):
                print(
                    f"sample {i + 1:05d}/{num_arch_samples:05d} "
                    f"acc={metrics['acc']:.2f} macs={macs/1e6:.2f}M "
                    f"time={t_eval - t0:.2f}s calib={t_calib - t0:.2f}s eval={t_eval - t_calib:.2f}s spec={spec}"
                )
            else:
                print(
                    f"sample {i + 1:05d}/{num_arch_samples:05d} "
    ...
    print(f"Total sampling time: {time.perf_counter() - total_t0:.2f}s")
...
def cmd_train_supernet(args: argparse.Namespace) -> None:
    ...
    for stage, n_epochs, subnets_per_batch, kd_ratio in schedule:
        ...
        if stage != "largest" and teacher_model is None:
            print("Creating frozen largest-subnet teacher for KD...", flush=True)
            teacher_model = make_frozen_teacher(model, device)
...
def cmd_sample_acc_dataset(args: argparse.Namespace) -> None:
    ...
    eval_options = parse_eval_options(args.eval_preset, args.eval_options)
...
def cmd_eval_subnet(args: argparse.Namespace) -> None:
    ...
    calib_loader: Iterable = train_loader if eval_options.get("bn_calib_loader", "train") == "train" else val_loader
    eval_loader: Iterable = test_loader
    if args.bn_calib_batches > 0 and eval_options.get("cache_calib_on_gpu", False):
        calib_loader = cache_batches_on_gpu(
            calib_loader, device, args.bn_calib_batches, model.search_space.resolutions,
            pre_resize=bool(eval_options.get("pre_resize_cache", False)), include_targets=False
        )
    if eval_options.get("cache_eval_on_gpu", False):
        eval_loader = cache_batches_on_gpu(
            eval_loader, device, args.test_batches, model.search_space.resolutions,
            pre_resize=bool(eval_options.get("pre_resize_cache", False)), include_targets=True
        )
        eval_max_batches = None
    else:
        eval_max_batches = args.test_batches

    t0 = time.perf_counter()
    if args.bn_calib_batches > 0:
        calibrate_bn(model, calib_loader, device, spec, num_batches=args.bn_calib_batches)
    ...
    print(f"Evaluation time: total={t2 - t0:.2f}s calib={t1 - t0:.2f}s eval={t2 - t1:.2f}s")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--eval-preset", type=str, default="balanced", choices=list(EVAL_OPTION_PRESETS.keys()))
    p.add_argument("--eval-options", type=str, default=None, help="JSON string or path overriding the eval option dictionary.")
    ...
    p.add_argument("--test-batches", type=int, default=None)
    p.add_argument("--eval-preset", type=str, default="balanced", choices=list(EVAL_OPTION_PRESETS.keys()))
    p.add_argument("--eval-options", type=str, default=None, help="JSON string or path overriding the eval option dictionary.")
