...
# Version avec le BN avec momentum 0.1, classique mais sur petit nombre de batch pose probleme car le premier terme compte trop
...
@torch.no_grad()
def calibrate_bn(model, loader, device, spec, num_batches=50):
...
def cmd_eval_subnet(args: argparse.Namespace) -> None:
    ...
    metrics = evaluate(model, test_loader, device, spec=spec, max_batches=args.test_batches)
